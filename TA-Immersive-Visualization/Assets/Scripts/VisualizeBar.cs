...
public class VisualizeBar : MonoBehaviour
{
    ...
    private const int TOTAL_VOTER_OVERALL = 150000000;
    private const int AVERAGE_VOTER_PROVINCE = 5000000; 
    ...
    private float normalizeValue(int value){
        float newValue;
        if (((int)province) != 0){
            newValue = (float)value * (chartHeight / AVERAGE_VOTER_PROVINCE);
            return newValue;
        }
        else{
            newValue = (float)value * (chartHeight / (TOTAL_VOTER_OVERALL));
            return newValue;
        }
    }
